...
export default function Settings() {
  ...
  const loadSettings = useCallback(async () => {
    setIsLoading(true);
    try {
      // Load per-category objects for convenience
      const imapObj = await Setting.object('imap');
      const intercomObj = await Setting.object('intercom');
      const filtersObj = await Setting.object('filters');
      const generalObj = await Setting.object('general');

      const merged = { ...defaultSettings, ...generalObj, ...imapObj, ...intercomObj, ...filtersObj };
      setSettings(merged);
      // Keep dbSettings as flattened list for compatibility
      const flat = [];
      Object.entries(merged).forEach(([k, v]) => flat.push({ key: k, value: v }));
      setDbSettings(flat);
    } catch (error) {
  ...
  const saveSettings = async () => {
    ...
    try {
      // Partition settings by category and send as objects
      const byCat = { imap: {}, intercom: {}, filters: {}, general: {} };
      for (const [key, value] of Object.entries(settings)) {
        const cat = key.startsWith('imap_') ? 'imap' : key.startsWith('intercom_') ? 'intercom' : (key.startsWith('filter_') || key === 'global_filters') ? 'filters' : 'general';
        byCat[cat][key] = value;
      }

      // Send per-category bulk saves
      for (const cat of Object.keys(byCat)) {
        const obj = byCat[cat];
        // skip empty
        if (!Object.keys(obj).length) continue;
        const resp = await Setting.saveObject(cat, obj);
        if (!resp || !resp.success) throw new Error(`Failed to save category ${cat}`);
      }
