...
// Register a wrapper so API routes are handled by Express; non-/api requests are forwarded to Vite.
...
(async () => {
  ...
  if (!isProd) {
    try {
      ...
      // ensure middleware is used for non-API routes
      app.use((req, res, next) => {
        if (req.url.startsWith('/api')) return next();
